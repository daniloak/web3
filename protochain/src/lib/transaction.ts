...
export default class Transaction {
    ...
    isValid(difficulty: number, totalFees: number) : Validation {
        ...
        if (this.txInputs && this.txInputs.length){
            ...
            const inputOutputSum = this.txOutputs.map(txo => txo.amount).reduce((a, b) => a + b, 0)

            if (inputSum < inputOutputSum){
                return new Validation(false, `Invalid tx: input amoungs must be equals or greater than outputs amoungs`)
            }
        }
