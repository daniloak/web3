...
export default class Blockchain {
    ...
    addBlock(block: Block) : Validation {
        ...
        const tsx = block.transactions.filter(tx => tx.type !== TransactionType.FEE).map(tx => tx.hash)
        const newMempool = this.mempool.filter(tx => !tsx.includes(tx.hash));

        if (newMempool.length + tsx.length !== this.mempool.length){
    ...
    //**
    // Get all inputs that received from my wallet (wallet's spent) */
    ...
    //**
    // Get all outputs to my wallet (wallet's earnings) */
